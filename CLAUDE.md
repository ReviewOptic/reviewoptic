...
**NEXT SESSION — FIRST STEPS:**
1. **Confirm whether the user checked/fixed the production `ADMIN_EMAIL` secret** — this is the most likely explanation for new signups not appearing as admin customers, but it's unverified since this sandbox can't inspect production environment variables.
2. Still pending from session 113: a real end-to-end login-as-team-member test of the stats/feedback scoping work, and a real Stripe test-webhook run to confirm the referral-credit relocation actually fires correctly.
3. `bucksandherts` — still unconfirmed, ask directly if it resurfaces (the "which header" half of this carryover was dropped per user request this session).
4. If a customer's follow-up delay timing looks like it "changed" unexpectedly, that's the session-114 `toCamel` fix working correctly (their real saved value is finally being used) — not a new bug, mentioned here again since it hasn't come up yet.
