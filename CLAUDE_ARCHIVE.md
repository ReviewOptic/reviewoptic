...
**NEXT SESSION — FIRST STEPS (superseded by later sessions, kept for history):**
1. Do a real end-to-end verification of this session's biggest changes — log in as a team member and confirm scoping; a real Stripe test-mode webhook run to confirm the relocated referral-credit logic actually fires.
2. Carry forward: get the saved template text for `cancellation` and `account_deletion` to strip stray hyperlinks — closed out session 114 (user confirmed resolved).
3. `[demo] Auto-reseed failed: column "follow_up1_days"...` — fixed properly in session 114.
4. `bucksandherts` and the "which header will you delete" question — dropped per user request, session 115.
...
**Also confirmed still-pre-existing, unrelated to this session's work:** dev server logs show `[demo] Auto-reseed failed: column "follow_up1_days" of relation "settings" does not exist` on every boot. Flagged to user, not touched. (Fixed properly in session 114 — turned out to be a real bug affecting every account's follow-up day settings, not just the demo account.)
