        ...
        <div>
          ...
          <div className="space-y-6">
            {FEATURES.map(item => {
              ...
              const isSocialCard = item.title === "Auto-post reviews to social media";
              const comingSoonTags = isChannelCard
                ? [!smsEnabled && "SMS", !whatsappEnabled && "WhatsApp"].filter(Boolean)
                : isSocialCard && !socialEnabled ? ["Coming soon"] : [];
              return (
                <div key={item.title} className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {comingSoonTags.length > 0 && (
                      <span className="inline-block mt-1 text-[11px] bg-white/10 text-white/70 rounded px-1.5 py-0.5">
                        {isSocialCard ? "Coming soon" : `${comingSoonTags.join(" & ")} coming soon`}
                      </span>
                    )}
                  </div>
                </div>
